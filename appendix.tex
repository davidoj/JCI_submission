...
\section{Equality of equally sized contractions}\label{sec:equal_condits}
...
\section{Symmetries of conditional probabilities}\label{sec:io_contract_proofs}

Example \ref{ex:no_implication} shows that neither locality nor exchange commutativity is implied by the other.
...
\section[Representation]{Representation of IO contractible models}\label{app:representation_proof}

This is the proof of Lemmas \ref{th:table_rep_kernel} and \ref{lem:ciid_yd} and Theorem \ref{th:any_infinite_sequence}. The following definitions are reproduced for convenience. Note that these proofs use the string diagram notation explained in Appendix \ref{ssec:mken_diagrams}.
...
\subsection{IO contractibility proofs}\label{sec:io_contract_models}
...
\section{Precedent}\label{sec:proof_precedent}

\begin{reptheorem}{th:construction_latent_inputs}
...
Take $B_g:=\mathrm{EP}(\mathrm{span}(A_g)\cap \Delta(X))$ where $\mathrm{EP}$ is the function that returns the extreeme points of a convex set (note that $A_g$ is the intersection of two convex sets). $(b_{ig})_{i\in |B_g|}$ some enumeration of the elements of $B_g$. For each $g\in \Delta(X)$, $i\in\mathbb{N}\cup\{c\}$ set $\prob{P}_\alpha^{\RV{X}_i|\RV{E}_i\RV{G}}(\cdot|e,g)=b_{eg}$. 

Define
\begin{align}
    d_{g\alpha} := \begin{cases}
                \prob{P}_C^{\RV{X}_1|\RV{G}}(\cdot|g) & \alpha\not\in C\\
                \prob{P}_{\alpha}^{\RV{X}_c|\RV{G}}(\cdot|g) & \alpha\in C
              \end{cases}
\end{align}

By construction, for each $d_{g\alpha}$ there is some set of weights $\{0\leq w^e{g \alpha}|e\in E\}$, $\sum_{e\in E} w^e_{g \alpha} = 1$ such that 
\begin{align}
    d_{g\alpha} = \sum_{e\in E} w^e_{g \alpha} b_{eg}
\end{align}
Let $o$ be some index that is not an element of $C$. Set
\begin{align}
    \prob{P}_\alpha^{\prime \RV{E}_i|\RV{G}}(e|g)= \begin{cases}
        w^e_{g o} & i\in\mathbb{N}\\
        w^e_{g \alpha} & i=c
    \end{cases} 
\end{align}
Then $i\in\mathbb{N}$ implies
\begin{align}
    \sum_{e\in E} \prob{P}_\alpha^{\prime \RV{X}_i|\RV{E}_i\RV{G}}(\{x\}|e,g)\prob{P}_\alpha^{\prime \RV{E}_1|\RV{G}}(\{e\}|g) &= \sum_{e\in E}b^x_{eg} w^e_{g o}\\
    &= \prob{P}_{\alpha}^{\RV{X}_i|\RV{G}}(\{x\}|g)
\end{align}
and
\begin{align}
    \sum_{e\in E} \prob{P}_\alpha^{\RV{X}_c|\RV{E}_c\RV{G}}(J|e,g)\prob{P}_\alpha^{\RV{E}_c|\RV{G}}(\{e\}|g) &= \sum_{e\in E}b_{eg} w^e_{g \alpha}\\
    &= \prob{P}_{\alpha}^{\RV{X}_c|\RV{G}}(\{x\}|g)
\end{align}

Finally, choose $\prob{P}_\alpha^{\prime \RV{E}|\RV{X}\RV{G}}$ such that $\RV{E}\CI^e\RV{X}|(\RV{G},\mathrm{id}_C)$ and $\prob{P}_\alpha^{\prime \RV{G}}=\prob{P}_\alpha^{\RV{G}}$.

Then
\begin{align}
    \prob{P}_{\alpha}^{\prime \RV{X}}(\{x_{\mathbb{N}\cup\{c\}}\}) &= \int_G \sum_{e\in E} \prob{P}_\alpha^{\prime \RV{E}_i|\RV{G}}(\{e\}|g)\prob{P}_\alpha^{\prime \RV{X}_i|\RV{E}_i\RV{G}}(\{x_i\}|e,g)\prob{P}_\alpha^{\prime \RV{G}}(\mathrm{d}g)\\
    &= \int_G \prob{P}_\alpha^{\RV{X}_i|\RV{G}}(\{x_i\}|g)\prob{P}_\alpha^{\RV{G}}(\mathrm{d}g)\\
    &= \prob{P}_\alpha^{\RV{X}}
\end{align}
\end{proof}

\begin{reptheorem}{th:latent_to_observable}
Given a decision model $(\prob{P}_\cdot,(C,\sigalg{C}),(\Omega,\sigalg{F})$ and sequences $(\RV{E}_i,\RV{X}_i,\RV{Y}_i)_{i\in\mathbb{N}\cup\{c\}}$, $(\RV{Z}_i)_{i\in \mathbb{N}}$ all taking values in discrete sets, suppose the observations $(\RV{E}_i,\RV{X}_i,\RV{Y}_i,\RV{Z}_i)_{i\in \mathbb{N}}$ are exchangeable for all options $\alpha\in C$ and pairs $(\RV{E}_i,(\RV{X}_i,\RV{Y}_i))$ share conditionally independent responses. Take $\RV{G}$ to be the directing random measure of the observations $\prob{P}_\alpha^{(\RV{E}_i,\RV{X}_i,\RV{Y}_i,\RV{Z}_i)_{i\in \mathbb{N}}}$ and $\RV{H}$ to be the directing random conditional of $(\prob{P}_\cdot, \RV{E}_i, (\RV{X}_i,\RV{Y}_i))$. 

Fix some $g^{XY}_{EZ}\in \Delta(X\times Y)^{EZ}$, and let $\prob{Q}_\alpha:= \prob{P}_\alpha^{\mathrm{id}_\Omega|\RV{G}^{XY}_{EZ}}(\cdot|g^{XY}_{EZ})$ -- i.e. $\prob{Q}_\alpha$ is $\prob{P}_\alpha$ conditioned on $\RV{G}^{XY}_{EZ}=g^{XY}_{EZ}$.

Suppose for any $i\in\mathbb{N}$, $\RV{Y}_i \CI^e_{\prob{Q}} \RV{Z}_i | (\RV{X}_i,\RV{G}^Y_{XZ}, \mathrm{id}_C)$ and for all $\alpha$, $\prob{Q}_\alpha$-almost all $z,z'\in Z$, $e\in E$, $g^e_{z}\in [0,1]$, $g^Y_{EXZ}\in $ satisfies the \emph{dominated posterior} assumption:
\begin{align}
    \prob{Q}_{\alpha}^{\RV{G}^E_{z'}|\RV{G}^E_{z}}(\cdot|g^e_{z}) \ll U_{[0,1]^{|E|}}\label{eq:lebesgue_dom_app}
\end{align} 

then $(\prob{Q}_\cdot,\RV{X},\RV{Y})$ is also IO contractible.
\end{reptheorem}
